...
const base = 'http://127.0.0.1:4173/Apple-orc/';
...
page.on('response',r=>{if(r.url().startsWith(base)&&r.status()>=400)networkErrors.push({url:r.url(),status:r.status()});});
page.on('requestfailed',r=>{if(r.url().startsWith(base)&&!r.failure()?.errorText.includes('ABORTED'))networkErrors.push({url:r.url(),failure:r.failure()});});
...
try{
  await check('workbench loads with all 13 presets',async()=>{await open('');assert.match(await page.title(),/Apple Orc/);assert.equal(await page.locator('.orc-preset').count(),13);});
  await check('preset selection and scene mode respond',async()=>{await page.locator('.orc-preset').nth(1).click();assert.equal(await page.locator('.orc-preset').nth(1).getAttribute('aria-pressed'),'true');await page.getByRole('radio',{name:'Scene',exact:true}).click();await page.waitForSelector('.orc-scene-pill-text');await page.getByRole('radio',{name:'Orb',exact:true}).click();});
  ...
  await check('gallery has six live components and state controls',async()=>{await open('showcase.html?view=presets');assert.equal(await page.locator('.live-preset-grid [data-renderer]').count(),6);await page.getByRole('button',{name:'Idle',exact:true}).click();await page.getByRole('button',{name:'Thinking',exact:true}).click();});
  await page.waitForTimeout(1500);await capture('03-presets');
  ...
  await check('no application exceptions or failed local assets',()=>{assert.deepEqual(errors,[]);assert.deepEqual(networkErrors,[]);});
}finally{
  const failed=results.filter(r=>!r.pass).length;
  await writeFile('evidence/browser.json',JSON.stringify({sourceCommit:process.env.GITHUB_SHA??'local',browser:await browser.version(),method:'Playwright screenshots of the production GitHub Pages bundle; no generated mockups or screenshot retouching',screenshotCount:screenshots.length,checked:results.length,passed:results.length-failed,failed,results,screenshots,errors,consoleErrors,warnings,networkErrors,limitations:['Chromium only; not Safari/iOS certification','GPU may be software-rendered; screenshot metadata records renderer','Local component examples; no model API, microphone or customer deployment']},null,2));
