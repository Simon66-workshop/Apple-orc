...
function InterfaceExample() {
  ...
  return <div className="showcase-interface">
    ...
    <section className="interface-example" aria-label="Live React integration"><div className="example-top"><span>Assistant interface</span><span className="local-label">Local demo</span></div><div className="example-orb"><AppleOrc preset={preset} state={state} size={260}/></div><h2>{state === "thinking" ? text || "Thinking" : "Ready when you are"}</h2><p>The component follows your app state.<br/>The rest of the conversation is yours.</p><div className="state-buttons" role="group" aria-label="Component state"><button aria-pressed={state === "idle"} onClick={() => setState("idle")}><Square size={14}/> Idle</button><button aria-pressed={state === "thinking"} onClick={() => setState("thinking")}><Play size={14}/> Thinking</button></div><div className="example-config"><label>Preset<select value={preset} onChange={e => setPreset(e.target.value as StyleName)}>{styleNames.map(s => <option key={s} value={s}>{styleLabels.en[s]}</option>)}</select></label><label>Status text<input value={text} maxLength={60} onChange={e => setText(e.target.value)}/></label></div></section>
