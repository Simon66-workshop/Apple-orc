...
function PresetLibrary() {
  const [state, setState] = useState<OrbStateName>("thinking");
  const [active, setActive] = useState<StyleName>("siri");
  return <section className="preset-showcase"><div className="section-heading"><div><h1>Same component.<br/><span>A different character.</span></h1><p>Six choices from 13 presets. The selected orb is live; the others are preset thumbnails.</p></div><div className="state-buttons" aria-label="Gallery state" role="group"><button aria-pressed={state === "idle"} onClick={() => setState("idle")}>Idle</button><button aria-pressed={state === "thinking"} onClick={() => setState("thinking")}>Thinking</button></div></div><div className="live-preset-grid">{gallery.map((preset, index) => <article key={preset} data-active={active === preset}><button className="preset-orb" data-preset={preset} aria-label={`Preview ${styleLabels.en[preset]}`} aria-pressed={active === preset} onClick={() => setActive(preset)}>{active === preset ? <AppleOrc preset={preset} state={state} size={170}/> : <img src={`${base}presets/${preset}.png`} width={170} height={170} alt=""/>}</button><div><span className="preset-number">0{index + 1}{active === preset ? " · LIVE" : ""}</span><h2>{styleLabels.en[preset]}</h2><code>{preset}</code></div></article>)}</div><a className="showcase-secondary" href={base}>Tune all 13 presets in the workbench <ArrowUpRight size={17}/></a></section>;
}
function EmbedLab() {
